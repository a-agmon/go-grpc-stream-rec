// Matrix factorization package by https://github.com/jbochi/facts
...
func (vm *VectorModel) ScoreCandidates(candidates []int, seenDocs map[int]bool) (scores []DocumentScore, err error) {
	candidateScores, err := vm.scoreCandidates(candidates, seenDocs)
	if err != nil {
		return nil, err
	}
...
func (vm *VectorModel) userVector(confidenceMap map[int]float64) (mat.VecDense, error) {
	...
	// var ch mat.Cholesky
	// if ok := ch.Factorize(&mat.Symmetric{A, vm.nFactors}); !ok {
